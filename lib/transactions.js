...
TxController.prototype.transformTransaction = function(transaction, options, callback) {
  ...
  if(transaction.height >= 0) {
    confirmations = this.node.services.rpgd.height - transaction.height + 1;
  }
...
TxController.prototype.transformOutput = function(options, output, index) {
  ...
  if (output.address) {
    ...
    var address = rpgcore.Address(output.address); //TODO return type from rpgcore-node
...
TxController.prototype.transformRPGTransaction = function(transaction) {
