...
UtilsController.prototype.estimateSmartFee = function(req, res) {
  ...
  async.map(nbBlocks, function(n, next) {
    var num = parseInt(n);
    // Insight and RPGcoin JSON-RPC return rpgcoin for this value (instead of satoshis).
    self.node.services.rpgd.estimateSmartFee(num, conservative, function(err, fee) {
      if (err) {
        return next(err);
      }
      next(null, [num, fee]);
