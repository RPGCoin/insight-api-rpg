...
/**
 * A service for RPGcore to enable HTTP routes to query information about the blockchain.
 *
...
var InsightAPI = function(options) {
  ...
  this.coinShort = options.coinShort || 'RPG';
  this.coinTicker = options.coinTicker || 'https://api.coinmarketcap.com/v1/ticker/rpgcoin/?convert=USD';
...
InsightAPI.dependencies = ['rpgd', 'web'];
...
InsightAPI.prototype.start = function(callback) {
  ...
  this.node.services.rpgd.on('tx', this.transactionEventHandler.bind(this));
  this.node.services.rpgd.on('block', this.blockEventHandler.bind(this));
...
InsightAPI.prototype.getPublishEvents = function() {
  return [
    ...
    },
    {
        ...
        scope: this,
        subscribe: function (emitter) {
            this.subscribe(emitter, 'rpg');
        }.bind(this),
        unsubscribe: function (emitter) {
            this.unsubscribe(emitter, 'rpg');
        },
        extraEvents: ['rpg/tx', 'rpg/block']
    }
  ];
};
...
InsightAPI.prototype.blockEventHandler = function(hashBuffer) {
  ...
  if (self.subscriptions.rpg.length) {
      ...
      this.blocksController.getBlockByHash(hashBuffer.toString('hex'), function (err, block) {
          ...
          async.mapSeries(block.tx, function(txid, next) {
              ...
              // Notify rpg subscribers
              for (var i = 0; i < self.subscriptions.rpg.length; i++) {
                  self.subscriptions.rpg[i].emit('rpg/block', {
                      block: block,
...
InsightAPI.prototype.transactionEventHandler = function(txBuffer) {
   ...
   if (this.subscriptions.inv.length || this.subscriptions.rpg.length) {
       ...
       if (this.subscriptions.rpg.length) {
           var transformedTrx = this.txController.transformRPGTransaction(tx);

           for (var i = 0; i < this.subscriptions.rpg.length; i++) {
               this.subscriptions.rpg[i].emit('rpg/tx', transformedTrx);
           }
