...
StatusController.prototype.getInfo = function(callback) {
  this.node.services.rpgd.getInfo(function(err, result) {
    if (err) {
      return callback(err);
    }
    var info = {
...
StatusController.prototype.getMiningInfo = function(callback) {
  this.node.services.rpgd.getMiningInfo(function(err, result) {
    if (err) {
      return callback(err);
    }
	var miningInfo = {
...
StatusController.prototype.getPeerInfo = function(callback) {
  this.node.services.rpgd.getPeerInfo(function(err, response) {
    if (err) {
      return callback(err);
    }
  var peers = [];
...
StatusController.prototype.getLastBlockHash = function() {
  var hash = this.node.services.rpgd.tiphash;
  return {
    syncTipHash: hash,
    lastblockhash: hash
  };
...
StatusController.prototype.getBestBlockHash = function(callback) {
  this.node.services.rpgd.getBestBlockHash(function(err, hash) {
    if (err) {
      return callback(err);
    }
    callback(null, {
...
StatusController.prototype.getDifficulty = function(callback) {
  this.node.services.rpgd.getInfo(function(err, info) {
    if (err) {
      return callback(err);
    }
    callback(null, {
...
StatusController.prototype.sync = function(req, res) {
  ...
  this.node.services.rpgd.isSynced(function(err, synced) {
    if (err) {
      return self.common.handleErrors(err, res);
    }
    if (synced) {
    ...
    self.node.services.rpgd.syncPercentage(function(err, percentage) {
      if (err) {
        return self.common.handleErrors(err, res);
      }
      var info = {
        status: status,
        blockChainHeight: self.node.services.rpgd.height,
        syncPercentage: Math.round(percentage),
        height: self.node.services.rpgd.height,
        error: null,
        type: 'rpgcore node'
      };
